...
SELECT ads.item_id,
       (ads.ad_spend / ads.clicks) AS cpc
FROM ads
WHERE ads.clicks > 0
ORDER BY cpc DESC
LIMIT 1;
"""
    ...
    normalized_question = user_input.lower().strip().rstrip("?.")
    if normalized_question in predefined_queries:
        generated_sql = predefined_queries[normalized_question].strip()
    else:
        # Select LLM and template
...
Geberate Query like this:
(if Calculate the RoAS (Return on Ad Spend).
...
- Always use fully qualified column names (e.g.,ads.ad_spend,ads.ad_sale) when referring to columns.
 ).
